...
def GGID_lookup(GG_ID, taxonomy_file=0):
    ...
    if(taxonomy_file == 0):
        taxonomy_file = '/home/ubuntu/databases/gg_13_5_otus/taxonomy/97_otu_taxonomy.txt'
...
def convert_GGID_dict_to_latin_names(alignment_dict):
    # Convert alignment dictionary with GreenGene IDs to latin names
    new_dict = {}
    for key in alignment_dict:
        assignments = alignment_dict[key]
        latin_names = [GGID_lookup(ass) for ass in assignments if ass != '*']
        new_dict[key] = latin_names
    return new_dict


def collapse_taxonomic_contents(OTU_table, taxonomic_level, parent_node=0, counts=False):
    ...
    # If counts is left unspecificed, it will return relative abundances by default.  Set counts=True
    # to collapse counts as well.
    ...
    # If OTU table contains 'unidentified' reads, remove those from the list of IDs.
    # They will be added at the end.
    OTU_IDs = [OTU_ID for OTU_ID in OTU_IDs if OTU_ID != 'unidentified']
...
def collapse_OTU_table(OTU_table, output_OTU_table, taxonomic_level, parent_node=0):
    # Collapse OTU table to given taxonomic level
    taxon_dict = collapse_taxonomic_contents(OTU_table, taxonomic_level, parent_node, counts=True)
    OTU_IDs = taxon_dict.keys()
    x = pd.DataFrame.from_dict(taxon_dict)
    x.transpose().to_csv(output_OTU_table, sep='\t', index_label='OTU_ID')
    return x.transpose()
